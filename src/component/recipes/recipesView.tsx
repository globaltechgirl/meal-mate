        ...
        <Box key={meal.id} style={styles.recipeCard}>
          <Box style={styles.mealImageWrapper}>
            ...
            <Box style={styles.starIcon} onClick={() => toggleStar(meal.id)}>
              {starred.includes(meal.id) ? <StarredIcon width={10} height={10} color="var(--light-200)" /> : <StarIcon width={10} height={10} color="var(--light-200)" />}
            </Box>
          </Box>
          ...
          <Box style={styles.recipeInfo}>
            <Popover width={100} position="bottom-end" shadow="md" withinPortal>
              <Popover.Target>
                <Box style={styles.menuIcon} onClick={(e) => e.stopPropagation()}>
                  <MenuIcon width={10} height={10} color="var(--light-200)" />
                </Box>
              </Popover.Target>
              <Popover.Dropdown style={{ width: 60, padding: 2, backgroundColor: "var(--dark-30)", border: "1px solid var(--dark-10)", borderRadius: 6, display: "flex", flexDirection: "column", gap: 2 }}>
