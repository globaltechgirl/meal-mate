...
const DetailsInstructions: FC = () => {
  ...
  const handleAdd = () => {
    setInstructions(prev => [...prev, { text: "", checked: false }]);
  ...
  const handleDelete = () => {
    if (selectedIndex !== null) {
      setInstructions(prev => prev.filter((_, i) => i !== selectedIndex));
  ...
  const updateText = (index: number, value: string) => {
    setInstructions(prev => prev.map((item, i) => (i === index ? { ...item, text: value } : item)));
  };
      ...
      <Box style={styles.detailsWrapper}>
        ...
        <Box style={styles.detailsMain}>
          {instructions.map((item, index) => (
            <Box style={styles.instructionsMain}>
              <Box key={index} style={styles.instructionRow}>
                ...
                {editingIndex === index ? (
                  ...
                  <Text style={styles.instructionText} onClick={() => setEditingIndex(index)}>
                    {item.text}
                  </Text>
                )}
              </Box>
